...
Point<point_size> lines_intersection(const Point<point_size> &p1_, const Point<point_size> &p2_, const Point<point_size> &p3_, const Point<point_size> &p4_) {
    ...
    // вычисляем угловые коэффициенты(при этом если один из отрезков горизонтален, то сразу ищем ответ)
    double k1, k2;
    if (p1.get_coord(0) == p2.get_coord(0)) {
        ...
        // проверяем, находится ли точка пересечения прямых нв отрезках
    ...
    //проверяем находится ли точка пересечения прямых на отрезках
